...
export const Description = () => {
    ...
    const { selected, catalog, tree } = useSelector(state => state.catalogReducer)
    ...
    const catalogItem = catalog.find(item => {
        const id = selected.type === 'catalog' ? selected.id : tree.find(tree_item => tree_item.id === selected.id).id_element
        return item.id === id        
    })
